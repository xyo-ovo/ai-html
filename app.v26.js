/* ============================================================
   AI HTML 工坊 · 核心逻辑 v26
   任意代码块都变文件卡片：
   · txt / md / json / py / css / js / docx 文本… 按语言定扩展名
   · html / svg 仍走全屏渲染预览，其余走源码查看
   ============================================================ */
...
const DEFAULT_GREETING = '先去设置页添加 API（DeepSeek / OpenAI / Kimi / 智谱等都可以），然后直接说「帮我写个 xxx 网页」。<br>代码块会直接变成文件卡片，点开就能看。<br>左上角 ☰ 可以看历史对话。';
...
async function mcpListTools(server) {
  try {
    await mcpRequest(server, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'ai-html-workshop', version: '26.0' },
    }, 0);
  } catch (e) {}
