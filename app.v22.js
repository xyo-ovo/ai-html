/* ============================================================
   AI HTML 工坊 · 核心逻辑 v22
   每对话独立人设（顶栏「AI 人设」编辑当前对话）
   · 移除全局系统提示词 · 世界书 · 工具 · 多会话
   ============================================================ */
...
function resetPersona() {
  if (!confirm('恢复默认人设？（当前对话的自定义人设会被清掉）')) return;
  const s = currentSession();
  if (s) {
    delete s.persona;
    LS.sessions = sessions;
...
function applyPersonaToAll() {
  if (!confirm('把当前这套人设设为所有新对话的默认？')) return;
  const p = sessionPersona();
  LS.persona = { avatar: p.avatar, name: p.name, bio: p.bio, greeting: p.greeting };
  toast('已设为新对话默认');
}
...
function buildSystemPrompt() {
  const base = sessionPersona().bio || DEFAULT_SYS;
...
async function mcpListTools(server) {
  try {
    await mcpRequest(server, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'ai-html-workshop', version: '22.0' },
    }, 0);
  } catch (e) {}
...
function newSession() {
  ...
  // 新对话继承当前这套人设
  s.persona = { avatar: cur.avatar, name: cur.name, bio: cur.bio, greeting: cur.greeting };
  sessions.unshift(s);
  LS.sessions = sessions;
  currentId = s.id;
  localStorage.setItem('aih.current', currentId);
...
const paAll = $('#pa-apply-all');
if (paAll) paAll.onclick = applyPersonaToAll;
