...
function applyTheme(t) {
  ...
  const btn = $('#btn-theme');
  if (btn) {
    const icon = t === 'dark' ? 'i-sun' : 'i-moon';
    btn.innerHTML = `<svg class="ic" style="display:block"><use href="#${icon}"/></svg>`;
  }
}
...
const LS = {
  ...
  get theme() { return localStorage.getItem('aih.theme') || 'light'; },
  set theme(v) { localStorage.setItem('aih.theme', v); },
...
function thinkEl(text, live, secs) {
  ...
  w.innerHTML = `<div class="think-head"><svg class="ic"><use href="#i-bulb"/></svg><span class="th-t">${label}</span><svg class="ic chev"><use href="#i-chev"/></svg></div><div class="think-body"><div class="tb-in"></div></div>`;
...
function codeEl(lang, code) {
  ...
  w.innerHTML = `<div class="code-head"><span class="lang"><svg class="ic"><use href="#i-code"/></svg>${esc(lang || 'code')}</span><button class="cp"><svg class="ic"><use href="#i-copy"/></svg>复制</button></div><pre><code>${esc(code)}</code></pre>`;
...
function fileEl(p) {
  ...
  d.innerHTML = `<div class="fc-icon"><svg class="ic"><use href="#i-globe"/></svg></div><div class="fc-meta"><div class="fc-name">${esc(p.name)}</div><div class="fc-sub">HTML 页面 · 点开预览</div></div><span class="fc-open"><svg class="ic"><use href="#i-eye"/></svg>打开</span>`;
...
function renderMsg(msg) {
  ...
  if (msg.parts || msg.reasoning) renderParts(w, msg);
  else if (msg.content) w.appendChild(textEl(msg.content));
  return w;
}
function updateStreaming(el, reasoning, text) {
  if (!streamView || streamView.el !== el) streamView = { el, think: null, tb: null, bubble: null };
  if (reasoning) {
    if (!streamView.think) {
      streamView.think = thinkEl('', true);
      el.appendChild(streamView.think);
      streamView.tb = streamView.think.querySelector('.tb-in');
    }
    streamView.tb.textContent = reasoning;
    const body = streamView.think.querySelector('.think-body');
    body.scrollTop = body.scrollHeight;
  }
  if (text) {
    if (!streamView.bubble) {
      streamView.bubble = document.createElement('div');
      streamView.bubble.className = 'bubble';
      el.appendChild(streamView.bubble);
    }
    streamView.bubble.textContent = text;
  }
}
function finishStreaming(el, secs) {
  const t = el.querySelector('.think');
  if (t) {
    t.classList.remove('live', 'open');
    t.querySelector('.th-t').textContent = '思考过程' + (secs ? ' · ' + secs + 's' : '');
  }
  streamView = null;
}
  ...
  try {
    ...
    for (;;) {
      ...
      while ((i = buf.indexOf('\n')) >= 0) {
        ...
        if (dc) acc += dc;
        if (rc || dc) { updateStreaming(el, accR, acc); scrollDown(); }
      }
    }
  } catch (e) {
    errMsg = e.message;
...
async function refreshFiles() {
  ...
  for (const f of list) {
    ...
    r.innerHTML = `<div class="fr-icon"><svg class="ic"><use href="#i-globe"/></svg></div>
      <div class="fr-meta"><div class="fr-name">${esc(f.name)}</div><div class="fr-sub">${new Date(f.ts).toLocaleString('zh-CN')}</div></div>
      <div class="fr-btns"><button class="op" title="预览"><svg class="ic"><use href="#i-eye"/></svg></button><button class="dl" title="下载"><svg class="ic"><use href="#i-download"/></svg></button><button class="rm" title="删除"><svg class="ic"><use href="#i-trash"/></svg></button></div>`;
...
function renderProviders() {
  ...
  for (const p of ps) {
    ...
    d.innerHTML = `<span class="pi-dot"></span><div class="pi-meta"><div class="pi-name">${esc(p.name)}</div><div class="pi-model">${esc(p.model || '未选模型')}</div></div><button class="edit" title="编辑"><svg class="ic"><use href="#i-edit"/></svg></button>`;
...
/* ---------- 主题按钮 ---------- */
$('#btn-theme').onclick = () => {
  const next = LS.theme === 'dark' ? 'light' : 'dark';
  LS.theme = next;
  applyTheme(next);
};
...
/* ---------- 初始化 ---------- */
(function init() {
  // 禁用 CSS 里的伪元素图标（由 JS 控制主题图标）
  const st = document.createElement('style');
  st.textContent = '#btn-theme::before{display:none!important}';
  ...
  if (!messages.length) {
    addEl(renderMsg({
      role: 'assistant',
      content: '嗨～我是 AI HTML 工坊\n\n先去右上角设置里添加你的 API（DeepSeek / OpenAI / Kimi / 智谱等都可以，填 Key 后能一键拉取模型列表）。\n\n然后直接说「帮我写个 xxx 网页」——我吐出来的 HTML 会自动变成文件卡片，点开就能全屏预览，右上角有 复制 / 下载 / 返回。\n\n推理模型（DeepSeek-R1、GLM 等）的思考过程会折叠显示，点「思考过程」就能展开查看。',
    }));
  }
})();
