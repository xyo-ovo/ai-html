/* ============================================================
   AI HTML 工坊 · 增强补丁 v4（extra.v3.js，再次换名绕缓存）
   · 关键修复：角色卡导入按钮「三重保险」绑定
     （直接 onclick + 事件委托 + 定时补绑）
   · 角色卡资料库：人设字段 / 世界书 / 正则脚本 三块分区
   · 世界书可开关、可复制成 ST JSON、可手动导入全局
   · 代码块行号+高亮 / 继续生成 / 编辑 AI 回复 / 对话搜索
   ============================================================ */

window.__EXTRA_VER = 'v34';

/* ================= 0. 依赖自检 ================= */
(function diag() {
  const ok = (typeof buildSystemPrompt === 'function') && (typeof renderMsg === 'function');
  window.__EXTRA_OK = ok;
  if (!ok) {
    console.warn('[extra.v3] app.v29.js 未就绪，部分功能不可用');
  }
})();
...
async function readPngChara(file) {
  ...
  if (!found) return { err: 'PNG 里没找到 chara / ccv3 数据块（可能不是角色卡，或已被压缩处理）' };
  return { val: found };
}
...
function updateDiag() {
  ...
  el.textContent = '脚本 ' + window.__EXTRA_VER +
    ' · 核心 ' + (appOk ? '✓' : '✗') +
    ' · 导入按钮 ' + (btnOk ? '✓' : '✗') +
    ' · 文件选择器 ' + (pickOk ? '✓' : '✗') +
    ' · 已绑定 ' + (_cardBound ? '✓' : '✗');
...
/* DOM 变化时再试 */
try {
  const mo = new MutationObserver(() => { bindCardImport(); });
