...
const DEFAULT_GREETING = '先去设置页添加 API（DeepSeek / OpenAI / Kimi / 智谱等都可以），然后直接说「帮我写个 xxx 网页」。<br>生成的 HTML 会直接变成文件卡片，点开全屏预览。<br>左上角 ☰ 可以看历史对话。';
...
function renderBrand() {
  ...
  const nm = $('#brand-name');
  if (av) av.textContent = p.avatar || DEFAULT_AVATAR;
  if (nm) nm.textContent = p.name || DEFAULT_NAME;
  try { document.title = p.name || DEFAULT_NAME; } catch (e) {}
}
function openPersona() {
  const p = LS.persona;
  $('#pa-avatar').value = p.avatar || DEFAULT_AVATAR;
  $('#pa-name').value = p.name || DEFAULT_NAME;
  $('#pa-bio').value = LS.sys || '';
...
function savePersona() {
  const p = LS.persona;
  p.avatar = ($('#pa-avatar').value || '').trim().slice(0, 4) || DEFAULT_AVATAR;
  p.name = ($('#pa-name').value || '').trim().slice(0, 24) || DEFAULT_NAME;
  p.greeting = ($('#pa-greeting').value || '').trim();
  LS.persona = p;
  LS.sys = ($('#pa-bio').value || '').trim();
  const sp = $('#sys-prompt');
  if (sp) sp.value = LS.sys;
  renderBrand();
  renderMessages();
  closeSheet('persona');
  toast('已保存');
...
function resetPersona() {
  if (!confirm('恢复默认人设？（头像 / 名字 / 人设 / 开场白都会重置）')) return;
  try { localStorage.removeItem('aih.persona'); } catch (e) {}
  LS.sys = '';
  const sp = $('#sys-prompt');
  if (sp) sp.value = '';
  renderBrand();
...
function saveMessages() {
  ...
  if (!s.titled && (!s.title || s.title === '新对话')) {
    ...
    if (firstUser) {
      const t = (firstUser.content || '').replace(/\s+/g, ' ').trim().slice(0, 22);
      if (t) s.title = t;
    }
  }
...
async function mcpListTools(server) {
  try {
    await mcpRequest(server, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'ai-html-workshop', version: '18.0' },
    }, 0);
  } catch (e) {}
...
function welcomeEl() {
  ...
  d.innerHTML = `<div class="w-icon">${esc(p.avatar || DEFAULT_AVATAR)}</div>
    <h3>${esc(p.name || DEFAULT_NAME)}</h3>
    <p>${html}</p>`;
  return d;
}
...
function renderStream(el, reasoning, segs, curText, live) {
  ...
  if (reasoning) {
    ...
    if (tb.textContent !== reasoning) {
      ...
      if (think.classList.contains('open')) tbody.scrollTop = tbody.scrollHeight;
    }
  } else if (think) {
    think.remove();
    think = null;
...
function renderConvList(kw) {
  ...
  for (const s of list) {
    ...
    d.innerHTML = `<div class="ci-meta">
        <div class="ci-title">${esc(s.title || '新对话')}</div>
...
function switchPage(name) {
  ...
  if (name === 'tools') { renderTools(); syncToolBtn(); }
  if (name === 'settings') { renderProviders(); renderAppearance(); }
...
$$('#preview, #provider-edit, #tool-edit, #persona').forEach(ov => {
...
async function refreshFiles() {
  ...
  for (const f of list) {
    ...
    r.innerHTML = `<div class="fr-icon">${icon('globe')}</div>
      <div class="fr-meta"><div class="fr-name">${esc(f.name)}</div><div class="fr-sub">${fmtTime(f.ts)}</div></div>
