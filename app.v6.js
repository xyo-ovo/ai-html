...
const LS = {
  ...
  get themeColor() { return localStorage.getItem('aih.themeColor') || 'violet'; },
...
function applyTheme() {
  ...
  const c = THEME_COLORS[LS.themeColor] || THEME_COLORS.violet;
  document.documentElement.style.setProperty('--acc', c[0]);
  document.documentElement.style.setProperty('--acc2', c[1]);
  const btn = $('#btn-theme');
  if (btn) btn.innerHTML = icon(isDarkActive() ? 'i-sun' : 'i-moon');
}
try {
...
$msgs.addEventListener('click', async e => {
  ...
  if (!msgEl || !msgEl.dataset.idx) return;
