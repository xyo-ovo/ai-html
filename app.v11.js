/* ============================================================
   AI HTML 工坊 · 核心逻辑 v11
   消息编辑 · token 统计 · 滚动放手 · MCP 工具 · 流式卡片化
   ============================================================ */
...
async function mcpListTools(server) {
  try {
    await mcpRequest(server, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'ai-html-workshop', version: '11.0' },
    }, 0);
  } catch (e) {}
...
function fmtUsage(u) {
  ...
  if (!total) return '';
  const txt = total >= 1000
    ? (total / 1000).toFixed(total >= 10000 ? 0 : 1).replace(/\.0$/, '') + 'k'
    : String(total);
  return txt + ' tokens';
}
...
function renderMsg(msg, idx) {
  ...
  if (msg.usage) {
    const u = fmtUsage(msg.usage);
    if (u) bits.push(`<span class="meta-usage">${u}</span>`);
  }
...
input.addEventListener('keydown', e => {
  ...
  if (e.key === 'Escape' && editingIndex !== null) { e.preventDefault(); cancelEdit(); }
});
