...
async function mcpListTools(server) {
  try {
    await mcpRequest(server, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'ai-html-workshop', version: '15.0' },
    }, 0);
  } catch (e) {}
...
function welcomeEl() {
  ...
  d.innerHTML = `<div class="w-icon">${icon('palette')}</div>
    <h3>AI HTML 工坊</h3>
    <p>先去设置页添加 API（DeepSeek / OpenAI / Kimi / 智谱等都可以），然后直接说「帮我写个 xxx 网页」。<br>生成的 HTML 会直接变成文件卡片，点开全屏预览。<br>左上角 ☰ 可以看历史对话。</p>`;
  return d;
}
...
function renderConvList(kw) {
  ...
  for (const s of list) {
    ...
    d.innerHTML = `<div class="ci-meta">
        ...
        <div class="ci-sub">${n} 条 · ${fmtDay(s.ts)}</div>
      </div>
...
$$('#preview, #provider-edit, #tool-edit').forEach(ov => {
...
$('#btn-clear-chat').onclick = () => {
  ...
  messages = [];
  const s = currentSession();
  if (s) { s.messages = messages; s.title = '新对话'; LS.sessions = sessions; }
  cancelEdit();
  renderMessages();
  toast('已清空');
};
