...
function reasoningParams(p) {
  ...
  const on = mode !== 'off';
  // DeepSeek 官方：思考由模型决定，不认开关，一律不传，避免报错
  ...
  // 通用兜底（硅基流动 / vLLM / Kimi 等 OpenAI 兼容服务）
...
function renderProviders() {
  ...
  for (const p of ps) {
    ...
    d.innerHTML = `<span class="pi-dot"></span><div class="pi-meta"><div class="pi-name">${esc(p.name)}</div><div class="pi-model">${esc(p.model || '未选模型')}${rTag}</div></div><button class="edit" title="编辑"><svg class="ic"><use href="#i-edit"/></svg></button>`;
