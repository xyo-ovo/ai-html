/* ============================================================
   AI HTML 工坊 · 核心逻辑 v8
   消息操作 · 暂停生成 · 流式卡片化 · 主题设置 · MCP 工具 · 零依赖
   ============================================================ */
...
function mcpHeaders(server) {
  ...
  try {
    const extra = JSON.parse(server.headers || '{}');
...
async function mcpListTools(server) {
  try {
    await mcpRequest(server, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'ai-html-workshop', version: '8.0' },
    }, 0);
  } catch (e) {}
...
async function testTool() {
  ...
  const headers = $('#te-headers').value.trim();
...
function saveTool() {
  ...
  if (headers) { try { JSON.parse(headers); } catch (e) { toast('请求头需要是合法 JSON'); return; } }
  const arr = LS.tools;
  if (editingToolId) {
