...
function saveMessages() {
  ...
  if (!s.titled && (!s.title || s.title === '新对话')) {
    ...
    if (firstUser) {
      ...
      if (t) s.title = t;
      else if (firstUser.attachments && firstUser.attachments.length) {
        const nm = firstUser.attachments[0].name || '';
        s.title = nm ? nm.slice(0, 22) : '图片 / 文件';
      }
    }
  }
...
async function mcpListTools(server) {
  try {
    await mcpRequest(server, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'ai-html-workshop', version: '29.0' },
    }, 0);
  } catch (e) {}
...
function fileEl(p) {
  ...
  const ic = p.preview === 'html' ? 'globe' : (p.preview === 'image' ? 'image' : 'code');
  d.innerHTML = `<div class="fc-icon">${icon(ic)}</div>
    ...
    <button class="fc-save" title="存入文件库">${icon('save')}</button>
    <span class="fc-open">${icon('eye')}打开</span>`;
  d.onclick = () => openPreview(p.fileId);
  const sv = d.querySelector('.fc-save');
  const markSaved = () => {
    sv.classList.add('on');
    sv.innerHTML = icon('check');
    sv.title = '已在文件库';
  };
  sv.onclick = async e => {
    e.stopPropagation();
    if (sv.classList.contains('on')) { toast('已经在文件库里了'); return; }
    const f = await dbGet(p.fileId);
...
function showPreview(f) {
  ...
  const imgBox = $('#pv-image');
  const codeBox = $('#pv-code');
  const codeStr = f.code || '';
  const isImg = f.preview === 'image' || /^data:image\//i.test(codeStr);
  const asHtml = !isImg && (f.preview === 'html' || /\.(html?|svg)$/i.test(f.name || ''));

  [frame, imgBox, codeBox].forEach(el => { if (el) el.classList.add('hidden'); });
  ...
  if (isImg) {
    ...
    if (frame) { frame.removeAttribute('srcdoc'); frame.style.height = ''; }
  } else if (asHtml) {
    ...
    frame.srcdoc = codeStr;
    frame.classList.remove('hidden');
  } else {
    if (frame) { frame.removeAttribute('srcdoc'); frame.style.height = ''; }
    if (codeBox) {
      codeBox.textContent = codeStr;
...
async function importFiles(files) {
  ...
  let n = 0;
  for (const f of list) {
    if (f.size > 12 * 1024 * 1024) { toast((f.name || '文件') + ' 太大（超过 12MB）'); continue; }
    const isImg = /^image\//.test(f.type || '') || /\.(png|jpe?g|gif|webp|bmp|avif|svg)$/i.test(f.name || '');
    try {
      if (isImg) {
        const dataUrl = await readAsDataURL(f);
        await dbPut({
          id: uid(), name: f.name || '图片', code: dataUrl, ts: Date.now(),
          preview: 'image', label: '图片', saved: true,
        });
        n++;
      } else {
        const ext = extOf(f.name);
        const okText = /^text\//.test(f.type || '') || TEXT_EXT.includes(ext);
        if (!okText) { toast('暂不支持 ' + (ext ? '.' + ext : '这种') + ' 文件'); continue; }
        const text = await readAsText(f);
        const k = fileKind(ext, text);
        await dbPut({
          id: uid(), name: f.name || '文件', code: text, ts: Date.now(),
          preview: k.preview, label: k.label, saved: true,
        });
        n++;
      }
    } catch (e) { toast('导入失败：' + (f.name || '')); }
  }
  if (n) {
    toast('已导入 ' + n + ' 个文件');
    refreshFiles();
...
async function refreshFiles() {
  ...
  const list = all.filter(f => f.saved !== false);
  const box = $('#file-list');
  box.innerHTML = '';
  if (!list.length) {
    box.innerHTML = '<div class="empty">文件库还是空的<br>导入文件，或在对话里点文件卡片上的「存入」</div>';
    return;
  }
  for (const f of list) {
    const r = document.createElement('div');
    r.className = 'file-row';
    const isImg = f.preview === 'image' || /^data:image\//i.test(f.code || '');
    const isHtmlFile = !isImg && (f.preview === 'html' || /\.(html?|svg)$/i.test(f.name || ''));
    const ic = isImg ? 'image' : (isHtmlFile ? 'globe' : 'code');
    r.innerHTML = `<div class="fr-icon">${icon(ic)}</div>
      <div class="fr-meta">
    ...
    r.querySelector('.rm').onclick = async () => {
      if (!confirm('从文件库删除「' + f.name + '」？')) return;
      if (f.saved === false) await dbDel(f.id);
      else { f.saved = false; await dbPut(f); }
      refreshFiles();
      toast('已移出文件库');
    };
    box.appendChild(r);
...
(function bindImport() {
  ...
  const pick = $('#pick-import');
  if (btn && pick) {
    btn.onclick = () => pick.click();
    pick.onchange = () => {
      if (pick.files && pick.files.length) importFiles(pick.files);
      pick.value = '';
    };
  }
})();
