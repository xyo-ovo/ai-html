/* ============================================================
   AI HTML 工坊 · 核心逻辑 v14
   思维链流式不重建（可关可滑）· 移动端 Enter 换行 · 底部导航多页面
   ============================================================ */
...
async function mcpListTools(server) {
  try {
    await mcpRequest(server, 'initialize', {
      ...
      capabilities: {},
      clientInfo: { name: 'ai-html-workshop', version: '14.0' },
    }, 0);
  } catch (e) {}
...
function welcomeEl() {
  ...
  d.innerHTML = `<div class="w-icon">${icon('palette')}</div>
    <h3>AI HTML 工坊</h3>
    <p>先去设置页添加 API（DeepSeek / OpenAI / Kimi / 智谱等都可以），然后直接说「帮我写个 xxx 网页」。<br>生成的 HTML 会直接变成文件卡片，点开全屏预览。</p>`;
  return d;
}
